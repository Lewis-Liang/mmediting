...
@PIPELINES.register_module()
class PairedRandomCropWithoutScale:
    """Paried random crop without scale.
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        lq_patch_size = self.gt_patch_size
        ...
        if h_gt != h_lq or w_gt != w_lq:
            raise ValueError(
                f'scale mismatches. GT ({h_gt}, {w_gt}) is not '
                f'scale of LQ ({h_lq}, {w_lq}).')
        if h_lq < lq_patch_size or w_lq < lq_patch_size:
            raise ValueError(
                f'LQ ({h_lq}, {w_lq}) is smaller than patch size '
                f'({lq_patch_size}, {lq_patch_size}). Please check '
        ...
        # the crop position corresponding to gt patch
