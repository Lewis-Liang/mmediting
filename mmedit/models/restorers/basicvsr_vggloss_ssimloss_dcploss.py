...
class BasicVSR_vggloss_ssimloss_dcploss(BasicVSR_vggloss_ssimloss):
    ...
    Args:
        ...
        dcp_loss (dict): Config for ldl loss. is_use_ema=True is required
    ...
    def __init__(self,
        ...
        super(). __init__(generator, pixel_loss, perceptual_loss, ssim_loss,
                 ...
                 test_cfg,
                 pretrained)
        assert (dcp_loss is None) or (dcp_loss and self.is_use_ema==True), "ldl loss requires ema"
        # dcp_loss
        self.dcp_loss = build_loss(dcp_loss)
    ...
    def forward_train(self, lq, gt):
        ...
        if self.dcp_loss:
            dcp_loss = self.dcp_loss(gt, output, output_ema)
            losses['loss_dcp'] = dcp_loss
